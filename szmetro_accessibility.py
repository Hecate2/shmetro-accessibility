...
from shmetro_accessibility import (
    AMAP_POI_URL,
    AMAP_TRANSIT_URL,
    AMapClient,
    RouteResult,
...
class ShenzhenStationHTMLParser(HTMLParser):
...
def load_station_catalog_from_html(html_path: Path) -> List[Station]:
    parser = ShenzhenStationHTMLParser()
...
def choose_station_queries(station: Station) -> List[str]:
    ...
    for station_name in station_name_variants(station.station_name):
        ...
        queries.extend(
            [
                f"{station_name} 深圳 {station.line_label} 地铁站",
                f"深圳 {station_name} {station.line_label} 地铁站",
                f"{station_name} {station.line_label} 地铁站",
                f"{station_name} 深圳 {station.line_label} 站",
                f"深圳地铁 {station.line_label} {station_name}",
                f"{station_name} 深圳 地铁站",
            ]
        )
...
def poi_type_score(station: Station, poi_type: str) -> Optional[Tuple[int, str]]:
    ...
    if "云巴" in station.line_label and any(keyword in poi_type for keyword in SPECIAL_RAIL_KEYWORDS):
...
def candidate_score(station: Station, poi: Dict[str, Any]) -> Tuple[int, str]:
    ...
    if adcode == SHENZHEN_ADCODE or "深圳" in f"{city_name}{district_name}{address}":
        score += 10
        reasons.append("shenzhen")
    ...
    if "云巴" in station.line_label and "云巴" in f"{name}{address}{poi_type}":
        score += 15
        reasons.append("yunba")

    return score, ",".join(reasons)


class ShenzhenAMapClient(AMapClient):
    ...
    async def route_transit(
        ...
        return await self._request_json(
            AMAP_TRANSIT_URL,
            {
                ...
                "city1": SHENZHEN_CITY_CODE,
                "city2": SHENZHEN_CITY_CODE,
                "strategy": strategy,
                "AlternativeRoute": "8",
                "nightflag": "0",
                "max_trans": "5",
...
def resolved_station_can_plan_route(record: ResolvedStation) -> bool:
    return record.status == "resolved" and bool(record.location) and bool(record.poi_id)


async def resolve_station_node(client: ShenzhenAMapClient, station: Station) -> ResolvedStation:
    best_record: Optional[ResolvedStation] = None

    for poi_types in choose_station_poi_types(station):
...
def write_station_catalog(stations: Sequence[Station], output_dir: Path) -> None:
...
def write_route_outputs(
    ...
    lines = [
        "# Directed Shenzhen Rail Travel Time Pairs",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Shenzhen rail accessibility crawler backed by AMap APIs")
    parser.add_argument("--output", default="output/shenzhen", help="Output directory")
    parser.add_argument(
        "--stations-html",
        default="深圳地铁车站列表 - 地铁通 MetroMan.html",
        help="MetroMan station-list HTML file",
    )
    parser.add_argument("--db-path", default="output/shenzhen/amap_transit.db", help="SQLite database path")
