...
def candidate_score(station: Station, poi: Dict[str, Any]) -> Tuple[int, str]:
    ...
    if adcode in choose_station_regions(station) or any(city in f"{city_name}{district_name}{address}" for city in station_city_names(station)):
        score += 10
        reasons.append("wuhan")
...
async def main() -> None:
    ...
    try:
        ...
        write_station_catalog(stations, output_dir, "Wuhan")
        ...
        write_station_catalog(stations, output_dir, "Wuhan")
        write_station_resolution(stations, resolved, output_dir)
        write_route_outputs(stations, routes, output_dir, "Wuhan")
