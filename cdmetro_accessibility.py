...
def choose_station_queries(station: Station) -> List[str]:
    ...
    line_label_simplified = station.line_label.replace("(支线)", "").replace("有轨电车", "").strip()
...
async def main() -> None:
    ...
    try:
        ...
        write_station_catalog(stations, output_dir, "Chengdu")
        ...
        write_station_catalog(stations, output_dir, "Chengdu")
        write_station_resolution(stations, resolved, output_dir)
        write_route_outputs(stations, routes, output_dir, "Chengdu")
