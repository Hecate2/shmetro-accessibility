...
- `output/stations_all.csv`
  ...
  - **Where it comes from:** this CSV is generated by the older
    `shmetro_accessibility_legacy.py` script, which scraped the
    Shanghai Metro mobile site.  If you don't already have the file,
    run the legacy crawler once before using the AMap-powered version:

      ```bash
      python3 shmetro_accessibility_legacy.py \
          --stations-csv output/stations_all.csv \
          --db-path /dev/null   # we only care about the station list
      ```

    (the legacy script may create its own database; you can toss it
