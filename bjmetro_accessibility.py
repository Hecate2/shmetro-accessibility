...
def choose_station_queries(station: Station) -> List[str]:
    ...
    for station_name in station_name_variants(station.station_name):
        ...
        queries.extend(
            [
                f"{station_name} 北京 {station.line_label} 地铁站",
                f"北京 {station_name} {station.line_label} 地铁站",
                f"{station_name} {station.line_label} 地铁站",
                f"{station_name} 北京 {station.line_label} 站",
                f"北京地铁 {station.line_label} {station_name}",
                f"{station_name} 北京 地铁站",
            ]
        )
...
def candidate_score(station: Station, poi: Dict[str, Any]) -> Tuple[int, str]:
    ...
    if adcode in choose_station_regions(station) or any(
        city in f"{city_name}{district_name}{address}" for city in station_city_names(station)
    ):
        score += 10
        reasons.append("beijing")
...
async def main() -> None:
    ...
    try:
        ...
        write_station_catalog(stations, output_dir, "Beijing")
        ...
        write_station_catalog(stations, output_dir, "Beijing")
        write_station_resolution(stations, resolved, output_dir)
        write_route_outputs(stations, routes, output_dir, "Beijing")
