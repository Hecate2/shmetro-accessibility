...
class ChangshaStationHTMLParser(HTMLParser):
...
def load_station_catalog_from_html(html_path: Path) -> List[Station]:
    parser = ChangshaStationHTMLParser()
...
class ChangshaAMapClient(AMapClient):
...
async def resolve_station_node(client: ChangshaAMapClient, station: Station) -> ResolvedStation:
...
async def crawl_routes(
    ...
    async def fetch_one(pair: Tuple[Station, Station]) -> RouteResult:
        ...
        payload = await client.route_transit(
            ...
            city1=station_city_code(origin),
            city2=station_city_code(destination),
...
def write_route_outputs(
    ...
    lines = [
        "# Directed Changsha Rail Travel Time Pairs",
        "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Changsha rail accessibility crawler backed by AMap APIs")
    parser.add_argument("--output", default="output/changsha", help="Output directory")
    parser.add_argument(
        "--stations-html",
        default="长沙地铁车站列表 - 地铁通 MetroMan.html",
        help="MetroMan station-list HTML file",
    )
    parser.add_argument("--db-path", default="output/changsha/amap_transit.db", help="SQLite database path")
...
async def main() -> None:
    ...
    client: Optional[ChangshaAMapClient] = None
    try:
